 ...
 * @since 1.0
 */
function espbw_sort_plugin_data( $a, $b ) {
 ...
 * @since 1.0
 */
function espbw_get_plugin_data() {
	...
	if( empty( $plugins_data ) ) {
		...
		if( is_wp_error( $plugins_data ) || empty( $plugins_data->plugins ) ) {
			...
			$file = ESPBW_DIR . '/includes/plugins-data.json';
		...
		if( ! is_wp_error( $plugins_data ) && ! empty( $plugins_data->plugins ) ) {
			...
			usort( $plugins_data->plugins, "espbw_sort_plugin_data" );

			set_transient( 'espbw_plugins_data', $plugins_data, (2 * HOUR_IN_SECONDS) );
		}
	}
